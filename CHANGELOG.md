...
+ [调整] Aicy 无限画布文本模型升级为 GPT-5.6 三模型，默认选择 Terra，文本请求改由 Aicy Gateway 转发至 Sub2API，失败时不再回退网页通道；生图默认尺寸调整为 auto。
+ [优化] 缩小画布节点工具栏并支持窄屏换行和边缘防溢出。
+ [修复] 统一画布图片来源解析，修复局部编辑和素材插入读取临时 URL 失败的问题。
